...
- `out_ωs::Vector{Float64}`: Strictly increasing energies for AC output
...
@doc raw"""
    DEAC_Cov(correlation_function::AbstractVector,
...
Runs the DEAC algorithm on the mean values passed in `correlation_function` using $\Chi^2$ fitting with the supplied covariance matrix. The covariance matrix is diagonalized and the fit is evaluated in its eigenbasis.
# Arguments
- `correlation_function::AbstractVector`: Mean input data in τ/ωₙ space
- `covariance_matrix::AbstractMatrix`: Covariance matrix of the mean correlation-function values. This must not be the covariance of individual, unaveraged bins
- `β::Float64`: Inverse temperature
- `input_grid::Vector{Float64}`: Values in τ or ωₙ space corresponding to `correlation_function`
- `out_ωs::Vector{Float64}`: Strictly increasing energies for AC output
- `kernel_type::String`: See below for allowable kernels
- `num_bins::Int64`: Bins to generate
- `runs_per_bin::Int64`: Number of runs per bin for statistics
...
# Optional Arguments
- `find_fitness_floor::Bool = false`: Find the floor for fitness and append that value to target fitnesses
- `population_size::Int64 = 20`: DEAC population size. Must be ≥ 6
- `base_seed::Int64 = 8675309`: Seed
- `number_of_generations::Int64 = 20_000`: Maximum number of mutation loops
- `keep_bin_data::Bool = true`: Save binned data or not
- `autoresume_from_checkpoint::Bool = false`: Resume from checkpoint if possible
- `verbose::Bool = false`: Print stats per run
- `fitness = 1.0`: Single value or an array of values for χ² fitting
...
- `self_adapting_differential_weight::Float64 = 0.9`: SAD
- `user_mutation! = nothing`: User passed function to add additional mutation to each iteration. See below for more information
- `eigenvalue_ratio_min::Float64 = 1e-8`: Cutoff to mask out eigenvectors with eigenvalues ≈ 0.0. Eigenvalues smaller than `eigenvalue_ratio_min * maximum(eigenvalues)` are excluded
...
If the arguments passed do not match those in the checkpoint the code will exit.
"""
function DEAC_Cov(correlation_function::AbstractVector,
                  covariance_matrix::AbstractMatrix,
                  β::Float64,
                  input_grid::Vector{Float64},
                  out_ωs::Vector{Float64},
    ...
    ngrid = length(correlation_function)
    length(input_grid) == ngrid || throw(DimensionMismatch(
        "correlation_function and input_grid must have the same length"))
    size(covariance_matrix) == (ngrid, ngrid) || throw(DimensionMismatch(
        "covariance_matrix must be a $ngrid × $ngrid matrix"))
    ishermitian(covariance_matrix) || throw(ArgumentError(
        "covariance_matrix must be symmetric (Hermitian for complex data)"))
    0.0 <= eigenvalue_ratio_min < 1.0 || throw(ArgumentError(
        "eigenvalue_ratio_min must be in [0, 1)"))
    ...
    return run_DEAC(
        (correlation_function, covariance_matrix),
        params,
        autoresume_from_checkpoint,
        keep_bin_data,
        find_fitness_floor,
...
function run_DEAC(Greens_tuple,
    ...
    normK = reshape(copy(ω_weights), 1, :)
    target_zeroth = 1.0
        ...
        Threads.@threads for thd in start_thread:total_runs
            ...
            lock(thread_lock) do 
                ...
                if finished_runs % params.runs_per_bin == 0
                    ...
                    bin_results!(bin_data,calculated_zeroth_moment,run_data,weight_data,curbin,ω_weights,Greens_tuple,fitness,seed_vec,generations,params)
